...
#==============================================================================#
                if text.lower().startswith('op '):
                  if msg._from in supperadmin:
                      MENTION = eval(msg.contentMetadata['MENTION'])
                      inkey = MENTION['MENTIONEES'][0]['M']
                      admin.append(str(inkey))
                      nadya.sendReplyMessage(msg_id,to, "เพิ่มสิทธิ์เรียบร้อยแล้ว(。-`ω´-)")
                if text.lower().startswith('deop '):
                  if msg._from in supperadmin:
                      MENTION = eval(msg.contentMetadata['MENTION'])
                      inkey = MENTION['MENTIONEES'][0]['M']
                      admin.remove(str(inkey))
                      nadya.sendReplyMessage(msg_id,to, "ลบสิทธิ์เรียบร้อยแล้ว(。-`ω´-)")
                if text.lower() == 'oplist':
                  if msg._from in supperadmin:
                    if admin == []:
                        nadya.sendReplyMessage(msg_id,to, "ไม่มีอำนาจ")
                    else:
                        nadya.sendReplyMessage(msg_id,to, "ต่อไปนี้เป็นผู้ตรวจสอบ")
                        mc = ""
                        for mi_d in admin:
