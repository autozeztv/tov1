...
def sendImage(to, path, name="image"):
    try:
        if settings["server"] == "VPS":
            alcohol.sendImageWithURL(to, str(path))
    except Exception as error:
        logError(error)
...
def mentionMembers2(to, mids=[]):
	...
	for point in range(parsed_len):
		...
		if result:
			if result.endswith('\n'): result = result[:-1]
			nadya.sendReplyMessage(msg.id, msg.to, result, {'MENTION': json.dumps({'MENTIONEES': mentionees})}, 0)
		result = ''
...
#==============================================================================# 
                ...
                if "รูปการ์ตูน " in msg.text.lower():
                    separate = msg.text.split(" ")
                    search = msg.text.replace(separate[0] + " ","")
                    with requests.session() as web:
                        web.headers["User-Agent"] = random.choice(settings["userAgent"])
                    ...
                    with requests.session() as web:
                        ...
                        for data in datas:
                            ret_ += "\n╠══[ {} ]".format(str(data["title"]))
                            ret_ += "\n╠❂͜͡☬➣https://www.youtube.com{}".format(str(data["href"]))
                        ret_ += "\n╚══[ จำนวนที่พบ {} ]".format(len(datas))
