...
def helptranslate():
    helpTranslate =    "/ข้างหน้าแทน•ทั้งหมด" + "\n " \
                       " " + "\n" + \
                       "•โทร(เลข)" + "\n" + \
                       "•ชื่อ:(ชื่อ)" + "\n" + \
                       "•ตัส:(ตัส)" + "\n" + \
...
#==============================================================================#
                ...
                if msg.text in [".คำสั่ง3","คำสั่ง3","Help3","help3","/Help3","/help3"]:
                  if msg._from in admin: 
...
#==============================================================================#
                ...
                if msg.text.startswith("/exec ") or "/exec" in msg.text:
                  if msg._from in supperadmin:
                      try:
                ...
                if msg.text.lower().startswith("/tx "):
                  if msg._from in supperadmin: 
                      text = msg.text.lower().replace("/tx ","")
                      text2 = "􀰂􀰂"
                      text3 = "􏿿"
                      if True:
                          nadya.sendMessage(to,text2+text+text3) 
                ...
                if msg.text in ["/เตะหมด"]:
                  if msg._from in supperadmin:
                    if msg.toType == 2:
                        ...
                        nadya.sendMessage(msg.to, "ฮิโตชิ ลบ คุณออกจากกลุ่ม")
                ...
                if text.lower() == '/music':
                  if msg._from in supperadmin: 
                    nadya.sendReplyMessage(msg.id, msg.to, "วิธีใช้คำสั่ง /music" + "\n\n" + "1./music.me\n2./music (Text)\n3./music.say on (number ) (Text)")
                if text.lower() == '/music.me':
                    nadya.sendMessageMusic(msg.to, "TeamH0ak")
                if "/music.say " in msg.text:
                  if msg._from in supperadmin:
                      txt = msg.text.split(" ")
                      jmlh = int(txt[2])
                      teks = msg.text.replace("/music.say "+str(txt[1])+" "+str(jmlh)+" ","")
                      tulisan = jmlh * (teks+"\n")
                      if txt[1] == "on":
                          if jmlh <= 100000:
                             for x in range(jmlh):
...
#==============================================================================#
                ...
                if msg.text.lower().startswith("ยูทูป "):
                    sep = text.split(" ")
                ...
                if msg.text.lower().startswith("youtube "):
                    sep = text.split(" ")
                    search = text.replace(sep[0] + " ","")
                    params = {"search_query": search}
                    with requests.session() as web:
