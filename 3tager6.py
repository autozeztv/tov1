...
def sendSticker(to, version, packageId, stickerId):
    ...
    nadya.sendReplyMessage(msg.id, msg.to, '', contentMetadata, 7)
...
#==============================================================================# 
                ...
                if "ขอเพลง " in msg.text.lower():
                     query = msg.text.split(" ")
                     try:
                         if len(query) == 3:
                             isi = yt(query[2])
                             hasil = isi[int(query[1])-1]
                             nadya.sendReplyMessage(msg.id, msg.to, hasil)
                         else:
                             isi = yt(query[1])
                             nadya.sendReplyMessage(msg.id, msg.to, isi[0])
                     except Exception as e:
                         nadya.sendReplyMessage(msg.id, msg.to, str(e))
...
#==============================================================================#
                ...
                if msg.text.lower().startswith(".ยกแชท "):
                  if msg._from in supperadmin: 
